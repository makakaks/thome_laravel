...
function initListener() {
    ...
    btnSubmit.addEventListener("click", async function () {
        ...
        if (title === "" || cover.files.length === 0) {
            window.showToast("กรุณาใส่ชื่อสิทธิพิเศษและอัปโหลดภาพหน้าปก", "error");
            return;
        }
        ...
        if (location.href.includes("edit")) {
            confirmDialog.confirmAction(
                ...
                async () => {
                    ...
                    if (res.ok) {
                        window.showToast("แก้ไขสิทธิพิเศษสำเร็จ", "success");
                        window.hideLoading();
                        ...
                        window.showToast(
                            "เกิดข้อผิดพลาดในการแก้ไขสิทธิพิเศษ",
                            "error"
                        );
                        window.hideLoading();
                    }
            ...
            confirmDialog.confirmAction(
                ...
                async () => {
                    ...
                    if (res.ok) {
                        window.showToast("สร้างสิทธิพิเศษสำเร็จ", "success");
                        window.hideLoading();
                        setTimeout(() => {
                            window.location.href = "/admin/privilege";
                        }, 2500);
                        ...
                        console.log("errorText: ", errorText);
                        window.showToast(
                            "เกิดข้อผิดพลาดในการสร้างสิทธิพิเศษ",
                            "error"
                        );
                        window.hideLoading();
                    }
